...
export default function MoneyDonationsView() {
  ...
  const fetchMoneyDonations = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || "Failed to load monetary donation inquiries.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const totalPledged = inquiries.reduce((acc, curr) => acc + (Number(curr.amount) || 0), 0);
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "1.5rem", flexWrap: "wrap", gap: "1rem" }}>
        <div>
          <h1 style={{ fontSize: "1.75rem", fontWeight: 900, color: "#1a2218", margin: 0 }}>
            Monetary Donation Inquiries
          </h1>
          <p style={{ fontSize: "0.875rem", color: "#6a7a64", marginTop: "0.25rem" }}>
            Track donation pledges, recurring contributors, and follow up with donors
          </p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          <div style={{ background: "#f0f7f0", padding: "0.4rem 1rem", borderRadius: 10, border: "1px solid #d4edd4", textAlign: "right" }}>
            <span style={{ fontSize: "0.6875rem", fontWeight: 700, color: "#8dc63f", textTransform: "uppercase" }}>Total Pledged</span>
            <div style={{ fontSize: "1.125rem", fontWeight: 900, color: "#2d6a2d" }}>₦{totalPledged.toLocaleString()}</div>
          </div>
          <button
            onClick={fetchMoneyDonations}
            style={{
              background: "#f0f7f0",
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "0.875rem" }}>
              <thead>
                <tr style={{ background: "#f8faf8", borderBottom: "1px solid #e8f0e8" }}>
                  ...
                  <th style={{ padding: "0.875rem 1.25rem", fontWeight: 700, color: "#4a5a44" }}>Pledged Amount</th>
                  <th style={{ padding: "0.875rem 1.25rem", fontWeight: 700, color: "#4a5a44" }}>Frequency</th>
              ...
              <tbody>
                {filtered.map((item) => (
                  ...
                  >
                    ...
                    <td style={{ padding: "1rem 1.25rem" }}>
                      <div style={{ fontSize: "1.0625rem", fontWeight: 900, color: "#2d6a2d" }}>
                        {item.amount ? `₦${Number(item.amount).toLocaleString()}` : "Custom amount"}
                      </div>
                    </td>
                    <td style={{ padding: "1rem 1.25rem" }}>
                      <span style={{ textTransform: "capitalize", fontWeight: 600, color: "#4a5a44", fontSize: "0.8125rem", background: "#f8faf8", padding: "0.25rem 0.6rem", borderRadius: 6, border: "1px solid #e8f0e8" }}>
